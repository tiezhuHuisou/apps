$(function(){
    ...
    $('.header_right').click(function() {											    						// 编辑按妞
        if( $(this).text() == '编辑' ) {																		// 点击之前为待编辑状态
            $(this).text('完成');	
//          $('.company_collect_list').find('a').attr('href','javascript:void(0);');							// 去掉链接
...
//          $('.company_collect_list').find('a').css('margin-left','0');										// 图片左侧的空隙删除
        }else {
            $(this).text('编辑');
//          $('.company_collect_list').find('a').attr('href','?g=app&m=company&a=detail');						// 去掉链接
...
//          $('.company_collect_list').find('a').css('margin-left','16px');										// 图片左侧的空隙添加
        }
    });
    ...
    $('.unselected_icon').click( function(){																	// 列表选中
    	if(!$(this).hasClass('allselected')){                                              					 	// 判断是全选还是单选
	    	if($(this).hasClass('selected_icon')){
	    		...
	    		$('.company_collect_list').each(function(){														// 得到选中的个数
	    		...
	    		if( count == $('.company_collect_list').length ){												// 全部选中
	    		...
	    		if(count == $('.company_collect_list').length){													// 若列表全选则底部全选按钮选中
					$('.allselected').addClass('selected_icon');
				}else{
					$('.allselected').removeClass('selected_icon');
				}
   ...
   $('.allselected').click( function(){														      				// 全选、反选
   		if( $(this).hasClass('selected_icon') ){																// 反选
	   	 	...
	   	 	$('.unselected_icon').removeClass('selected_icon')					// 列表不能选中
	   	 	$('.news_delete').removeClass('news_delete_selected');												// 删除按钮背景色改变
	   	}else{																									// 全选																																									
	   	 	$(this).addClass('selected_icon');
	   	 	$('.unselected_icon').addClass('selected_icon')	
