$( function() {
    $('.repassword_content_password_see').click(function(){                            //可见按钮点击事件
        if ( $(this).hasClass('nosee') ){                                            //可见按钮包含class('nosee')
            $(this).removeClass('nosee');                                            //移除class        
            $(this).siblings('.repassword_content_password_input').attr("type","text");               //密码可见
        } else {
            $(this).addClass('nosee');                   //添加class 
             $(this).siblings('.repassword_content_password_input').attr("type","password");           //密码不可见
        }
    });
	var times = 60;															//初始化倒计时的时间
	$(".repassword_content_valid_btn").click( function(){				    //点击发送，开始倒计时
		$(this).hide();
...
$('.repassword_btn').click(function(){
	var _form = $('form');
	$.post('',_form.serialize(),function(data){
		if(data.status==1){            
...
function sendTime(times){												    //倒计时函数
	if( times == 0 ) {
		$(".repassword_content_valid_btn").show().text("重新发送");
		$(".sent").hide();
	} else {
		times--;
