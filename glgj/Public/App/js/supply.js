        ...
        　　var windowHeight = $(this).height();                                // 当前可视的页面高度
            ...
            if ( scrollTop + windowHeight >= scrollHeight - expectHeight ) {    // 距离顶部+当前高度 >=文档总高度 即代表滑动到底部
                ...
                if ( ajaxFlag ) {
                    ajaxFlag = false;
                    $('.loading').show();
                    $.get('?g=app&m=index&a=ajaxsupplylist',data,function(data) {
                        $('.loading').hide();
                        if ( !data.length ) {
                            ajaxFlag = false;
                            return false;
