...
class CircleCommentModel extends Model{
    ...
    protected $_validate = array(
        ...
        array('pid', 'checkPid', '行业圈不存在或已被删除', self::MUST_VALIDATE, 'callback'),
        array('token', 'require', '请先登陆', self::MUST_VALIDATE),
        array('token', 'checkToken', '登陆信息已过期', self::MUST_VALIDATE, 'callback'),
        array('token', 'checkMine', '不能对自己进行回复', self::MUST_VALIDATE, 'callback'),
    ...
    protected function checkPid() {
        ...
        $result = M('Circle')->where(array('id'=>$pid))->getField('id');
        if ( $result ) {
            return true;
        }
        return false;
    ...
    protected function checkMine() {
        ...
        if ( $cid ) {
            $commentUid = $this->where(array('id'=>$cid))->getField('uid');
            if ( $commentUid == $uid ) {
                return false;
            }
        }
