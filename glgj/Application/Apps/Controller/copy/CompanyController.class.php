...
class CompanyController extends ApiController {
    ...
    public function index() {
        if ( IS_GET ) {
            ...
            !empty($title) && $where['name'] = array('like', '%'.$title.'%');
            $cid > 0 && $where['company_category_id'] = array('eq', $cid);
            $where['status'] = array('eq', 1);
            $list['company_list'] = M('Company')->field('id company_id,name,logo,business')->where($where)->order('flags DESC,sort DESC,modify_time DESC')->limit($page,10)->select();
            ...
            foreach ($list['company_list'] as $key => $value) {
                $list['company_list'][$key]['business'] && $list['company_list'][$key]['business'] = '主营行业：' . $list['company_list'][$key]['business'];
            }
