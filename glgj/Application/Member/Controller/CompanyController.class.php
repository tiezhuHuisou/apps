...
class CompanyController extends MemberController {
    public function _initialize() {
        parent::_initialize();
        /*获取公司ID*/
        $field = 'l.*,subphone,province_id,r.city_id,countyN,address,contact_user,contact_office,sex,site,qq,telephone,subphone,fax,email,r.modify_time,point,wechat,r.company_id';
    ...
    public function index() {
        ...
        if (IS_POST) {
            ...
            if (!$company_info) {
                $this->error('完善公司信息后才能上传网点', '?g=member&m=sale&a=company', 1);
            }
            $result = $model->update();
            if ($result) {
        ...
        if ($id) {
            /* 商品详细信息 */
            $condition['p_id'] = $id;
            $info = $model->getOneInfo($condition);
            $info['p_area'] = explode(',', $info['p_area']);
            $info['lng'] = $info['p_area'][0];
            $info['lat'] = $info['p_area'][1];
            $info['companyName'] = $this->company_info['name'];
        } else {
            $info['c_id'] = $this->company_info['id'];
            $info['companyName'] = $this->company_info['name'];
        }
    ...
    public function point() {
        $this->company_info['id'];
        $where['c_id'] = array('eq', $this->company_info['id']);
        $order = 'p_addtime desc';
        $list = $this->lists('CompanyPoint', $where, $order);
        $this->assign('list', $list);
        $this->assign('header', 'company');
        $this->assign('nav', 'point');
        $this->display();
    }
    ...
    public function pointDel() {
        $where['p_id'] = array('eq', $_GET['id']);
        $status = D('CompanyPoint')->delInfo($where);
        if ($status) {
            $this->success('删除成功');
        } else {
    ...
    public function contact() {
        if (IS_POST) {
            ...
            $linkStatus = $companyLinkModel->test();
            if (!$linkStatus) {
            ...
            if ($status) {
                ...
                if ($company_type != 1) {
                    ...
                    $this->success('保存企业资料成功', 'index.php?g=member&m=index');
                }
                $this->success('保存企业资料成功请等待审核', 'index.php?g=member&m=index');
            } else {
        ...
        /*企业联系信息*/
        $company_info = $this->company_info;
        $point = explode(',', $company_info['point']);
        $company_info['lng'] = $point[0];
        $company_info['lat'] = $point[1];
