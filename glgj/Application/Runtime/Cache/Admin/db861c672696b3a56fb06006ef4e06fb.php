...
<body>
	...
	<!-- 主体 开始 -->
	<div class="main">
		<input id="url" type="hidden" value="?g=admin&m=member&a=delall">
		<div class="wrapper">
			<!-- 主体头部 -->
			<div class="header">
				<h2>会员管理中心</h2>
				<a class="hover" href="?g=admin&m=member">会员列表</a>
				<a href="?g=admin&m=member&a=add">添加会员</a>
			</div>
			<!-- 主体搜索 -->
			<div class="search">
				<form class="form-horizontal" name="" action="?g=admin&m=member" method="post">
					<p>检索：</p>
					<select name="group">
						<option value="">全部</option>
						<?php if(is_array($group)): $i = 0; $__LIST__ = $group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gr): $mod = ($i % 2 );++$i;?><option value="<?php echo ($gr["gid"]); ?>" <?php if(($gr["gid"]) == $gid): ?>selected<?php endif; ?>><?php echo ($gr["gname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
			...
			<table class="data">
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
						<td><input class="check_box" type="checkbox" name="items" value="<?php echo ($li["uid"]); ?>" /></td>
						<td><?php echo ($li["uid"]); ?></td>
						<td>
							<span><?php echo ($li["mphone"]); ?></span>
							<span>(<span><?php echo ($li["gname"]); ?></span>)</span>
						</td>
						<td><?php echo ($li["name"]); ?></td>
						<td><?php echo (date("Y-m-d H:i",$li["regdate"])); ?></td>
						<td class="data_opt">
							<a href="?g=admin&m=member&a=add&id=<?php echo ($li["uid"]); ?>">修改</a>
							<a class="confirm ajax-get" href="index.php?g=admin&m=member&a=del&id=<?php echo ($li["uid"]); ?>">删除</a>
						</td>
					</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			</table>
			<!-- 操作 -->
			<div class="opt">
				<a class="opt_btn" href="?g=admin&m=member&a=add">添加</a>
