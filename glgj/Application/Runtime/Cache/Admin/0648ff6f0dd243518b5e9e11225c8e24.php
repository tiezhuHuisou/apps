...
<body>
	...
	<!-- 主体 开始 -->
	<div class="main">
	<input id="url" type="hidden" value="?g=admin&m=user&a=delall">
		<div class="wrapper">
			<!-- 主体头部 -->
			<div class="header">
				<h2>管理员管理中心</h2>
				<a class="hover" href="?g=admin&m=user">管理员列表</a>
				<a href="?g=admin&m=user&a=add">添加管理员</a>
			</div>
			<!-- 主体数据 -->
			<table class="data">
				<!-- 数据头部 -->
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="check_box" type="checkbox" name="items" value="<?php echo ($li["user_id"]); ?>" /></td>
					<td><?php echo ($li["user_id"]); ?></td>
					<td class="tl"><?php echo ($li["user_name"]); ?></td>
					<td class="tl"><?php echo ($li["nickname"]); ?></td>
					<td><?php echo (date("Y-m-d H:i",$li["add_time"])); ?></td>
					<td class="data_opt">
						<a href="?g=admin&m=user&a=add&user_id=<?php echo ($li["user_id"]); ?>">修改</a>
						<a class="confirm ajax-get" href="index.php?g=admin&m=user&a=del&id=<?php echo ($li["user_id"]); ?>">删除</a>
					</td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			</table>
			<!-- 操作 -->
			<div class="opt">
				<a class="opt_btn" href="?g=admin&m=user&a=add">添加</a>
