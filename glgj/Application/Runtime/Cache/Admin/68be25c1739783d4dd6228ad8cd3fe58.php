...
<body>
	...
	<!-- 主体 开始 -->
	<div class="main">
	<input id="url" type="hidden" value="?g=admin&m=comment&a=delall">
		<div class="wrapper">
			<!-- 主体头部 -->
			<div class="header">
				<h2>留言信息</h2>
			...
			<table class="data">
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
					...
					<td class="tl"><?php echo ($li["content"]); ?></td>
					<td><?php echo ($li["name"]); ?></td>
					<td><?php if($li["sex"] == 1): ?>男<?php else: ?>女<?php endif; ?></td>
					<!-- <td><?php echo ($li["email"]); ?></td> -->
					<td><?php echo ($li["mphone"]); ?></td>
					<td><?php echo (date("Y-m-d H:i",$li["addtime"])); ?></td>
					<td class="data_opt">
						<a class="confirm ajax-get" href="index.php?g=admin&m=comment&a=del&id=<?php echo ($li["id"]); ?>">删除</a>
					</td>
