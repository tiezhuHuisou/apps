...
<!-- 主体 开始 -->
<div class="main">
    <input id="url" type="hidden" value="?g=admin&m=product&a=goodsDelAll">
    <div class="wrapper">
        <!-- 主体头部 -->
        <div class="header">
            <h2>产品管理中心</h2>
            <a class="hover" href="?g=admin&m=product&a=goods">货物管理</a>
            <a href="?g=admin&m=product&a=goodsAdd">添加货物</a>
        </div>
        <!-- 主体搜索 -->
        <!--<div class="search">-->
            <!--<form class="form-horizontal" name="" action="?g=admin&m=product" method="post">-->
                ...
                <!--<select name="flags">-->
                    ...
                    <!--<?php if(is_array($flags)): $i = 0; $__LIST__ = $flags;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$fl): $mod = ($i % 2 );++$i;?>-->
                        <!--<option value="<?php echo ($fl["att"]); ?>"-->
                        <!--<?php if(($f) == $fl["att"]): ?>selected<?php endif; ?>-->
                        <!--&gt;<?php echo ($fl["attname"]); ?></option>-->
        ...
        <table class="data">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><?php echo ($li["provincen"]); echo ($li["cityn"]); echo ($li["countryn"]); ?></td>
                    <td><?php echo ($li["provincen2"]); echo ($li["cityn2"]); echo ($li["countryn2"]); ?></td>
                    <td><?php echo ($li['uid']); ?></td>
                    <td><?php echo ($li['name']); ?></td>
                    <td><?php echo ($li['transport_id']); ?></td>
                    <td><?php echo ($li['end_time']); ?></td>
                    <td><?php echo (date('Y-m-d',$li['create_time'])); ?></td>

                    <td class="data_opt">
                        <a href="?g=admin&m=product&a=goodsAdd&id=<?php echo ($li["id"]); ?>">修改</a>
                        <!--<a href="?g=admin&m=product&a=comment&id=<?php echo ($li["id"]); ?>&type=1">评价管理</a>-->
                        <a class="confirm ajax-get" href="index.php?g=admin&m=product&a=goodsDel&id=<?php echo ($li["id"]); ?>">删除</a>
                    </td>
        ...
        <!--<div class="opt">-->
            ...
            <!--<select name="" id="sel" style="margin-top:1px;">-->
                <!--<?php if(is_array($classify)): $i = 0; $__LIST__ = $classify;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cl): $mod = ($i % 2 );++$i;?>-->
