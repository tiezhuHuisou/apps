...
<body>
	...
	<!-- 主体 开始 -->
	<div class="main">
	<input id="url" type="hidden" value="?g=admin&m=user&a=group_delall">
		<div class="wrapper">
			<!-- 主体头部 -->
			<div class="header">
				<h2>管理员分组管理中心</h2>
				<a class="hover" href="?g=admin&m=user&a=group">管理员分组列表</a>
				<a href="?g=admin&m=user&a=group_add">添加管理员分组</a>
			</div>
			<!-- 主体数据 -->
			<table class="data">
				<!-- 数据头部 -->
				<tr class="data_header">
					...
					<th>ID</th>
					<th class="tl">管理员组组名</th>
					<th>添加日期</th>
					<th>操作</th>
				</tr>
				<!-- 一条数据开始 -->
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="check_box" type="checkbox" name="items" value="<?php echo ($li["gid"]); ?>" /></td>
					<td><?php echo ($li["gid"]); ?></td>
					<td class="tl"><?php echo ($li["gname"]); ?></td>
					<td><?php echo (date("Y-m-d H:i",$li["addtime"])); ?></td>
					<td class="data_opt">
						<a href="?g=admin&m=user&a=group_add&id=<?php echo ($li["gid"]); ?>">修改</a>
						<a class="confirm ajax-get" href="index.php?g=admin&m=user&a=group_del&id=<?php echo ($li["gid"]); ?>">删除</a>
					</td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			</table>
			<!-- 操作 -->
			<div class="opt">
				<a class="opt_btn" href="?g=admin&m=user&a=group_add">添加</a>
