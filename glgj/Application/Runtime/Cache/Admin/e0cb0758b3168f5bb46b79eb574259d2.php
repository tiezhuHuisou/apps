...
<body>
	...
	<!-- 主体 开始 -->
	<div class="main">
	<input id="url" type="hidden" value="?g=admin&m=push&a=delall">
		<div class="wrapper">
			<!-- 主体头部 -->
			<div class="header">
				<h2>推送消息管理中心</h2>
				<a class="hover" href="?g=admin&m=push">推送消息列表</a>
				<a href="?g=admin&m=push&a=add">添加推送消息</a>
			</div>
			<!-- 主体数据 -->
			<table class="data">
				<!-- 数据头部 -->
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
					...
					<td class="tl"><?php echo ($li["title"]); ?></td>
					<td class="tl"><a href="<?php echo ($li["url"]); ?>"><?php echo ($li["url"]); ?></a></td>
					<td><?php echo (date("Y-m-d H:i:s", $li["addtime"])); ?></td>
					<td class="data_opt">
						<!-- <a href="?g=admin&m=push&a=add&id=<?php echo ($li["id"]); ?>">修改</a> -->
						<a class="confirm ajax-get" href="index.php?g=admin&m=push&a=del&id=<?php echo ($li["id"]); ?>">删除</a>
					</td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			</table>
			<!-- 操作 -->
			<div class="opt">
				<a class="opt_btn" href="?g=admin&m=push&a=add">添加</a>
