...
<body>
	...
	<!-- 主体 开始 -->
	<div class="main">
		<input id="url" type="hidden" value="?g=admin&m=news&a=delall">
		<div class="wrapper">
			<!-- 主体头部 -->
			<div class="header">
				<h2>资讯管理中心</h2>
				<a class="hover" href="?g=admin&m=news">资讯列表</a>
				<a href="?g=admin&m=news&a=add">添加资讯</a>
			</div>
			<!-- 主体搜索 -->
			<div class="search">
				<form class="form-horizontal" name="" action="?g=admin&m=news" method="post">
					<p>筛选：</p>
					<select name="flags">
						<option value="">全部</option>
						<?php if(is_array($flags)): $i = 0; $__LIST__ = $flags;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$fl): $mod = ($i % 2 );++$i;?><option value="<?php echo ($fl["flags"]); ?>" <?php if(($f) == $fl["flags"]): ?>selected<?php endif; ?>><?php echo ($fl["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
					<p>搜索：</p>
			...
			<table class="data">
				...
				<tr class="data_header">
					...
					<th id="classify">ID</th>
					<th class="tl">文章标题</th>
					<th class="tl">显示</th>
					<th>更新日期</th>
					<th>操作</th>
				</tr>
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><!-- 一条数据开始 -->
					<tr>
						<td><input class="check_box" type="checkbox" name="items" value="<?php echo ($li["id"]); ?>" /></td>
						<td><?php echo ($li["id"]); ?></td>
						<td class="tl">
							<span>[<?php echo ($li["classify"]); ?>]</span>
							<span><?php echo ($li["title"]); ?></span>
							<?php if(!empty($li["image"])): ?><span class="green">(图)</span><?php endif; ?>
						</td>
						<td class="tl"><?php echo ($li['is_display']?"显示":"不显示"); if(!empty($li["flags"])): ?>&nbsp<span class="red">(<?php echo ($li["flags"]); ?>)<?php endif; ?></span></td>
						<td><?php echo (date("Y-m-d H:i:s",$li["uptime"])); ?></td>
						<td class="data_opt">
							<a href="?g=admin&m=news&a=add&id=<?php echo ($li["id"]); ?>">修改</a>
							<a class="confirm ajax-get" href="index.php?g=admin&m=news&a=del&id=<?php echo ($li["id"]); ?>">删除</a>
						</td>
					</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			</table>
			<!-- 操作 -->
			<div class="opt">
				<a class="opt_btn" href="?g=admin&m=news&a=add">添加</a>
