...
<script type="text/javascript" src="./Public/static/js/check.js"></script>
    <script type="text/javascript" src="./Public/Admin/js/classify.js"></script>
...
<body>
    ...
    <!-- 主体 开始 -->
    <div class="main">
        <input id="url" type="hidden" value="?g=admin&m=news&a=classify_delall">
        <div class="wrapper">
            <!-- 主体头部 -->
            <div class="header">
                <h2>资讯分类管理中心</h2>
                <a class="hover" href="?g=admin&m=news&a=classify">分类列表</a>
                <a href="?g=admin&m=news&a=classify_add">添加分类</a>
            </div>
            <!-- 主体数据 -->
            <table class="data">
                <!-- 数据头部 -->
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
                        <td><input class="check_box" type="checkbox" name="items" value="<?php echo ($li["cid"]); ?>" /></td>
                        <td class="tl"><?php echo ($li["cid"]); ?></td>
                        <td class="tl"><?php echo ($li["cname"]); ?></td>
                        <td><?php echo (date("Y-m-d H:i:s",$li["addtime"])); ?></td>
                        <td class="data_opt">
                            <a href="?g=admin&m=news&a=classify_add&cid=<?php echo ($li["cid"]); ?>">修改</a>
                            <a class="confirm ajax-get" href="index.php?g=admin&m=news&a=classify_del&cid=<?php echo ($li["cid"]); ?>">删除</a>
                        </td>
                    </tr>
                    <?php if(!empty($li["_child"])): if(is_array($li["_child"])): $i = 0; $__LIST__ = $li["_child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lii): $mod = ($i % 2 );++$i;?><tr class="no <?php echo ($li["cid"]); ?> p<?php echo ($li["cid"]); ?>">
                                <td><input class="check_box" type="checkbox" name="items" value="<?php echo ($lii["cid"]); ?>" /></td>
                                <td class="tl ti20">
                                    <i class="flex"></i>
                                    <span><?php echo ($lii["cid"]); ?></span>
                                </td>
                                <td class="tl"><?php echo ($lii["cname"]); ?></td>
                                <td><?php echo (date("Y-m-d H:i:s",$lii["addtime"])); ?></td>
                                <td class="data_opt">
                                    <a href="?g=admin&m=news&a=classify_add&cid=<?php echo ($lii["cid"]); ?>">修改</a>
                                    <a class="confirm ajax-get" href="index.php?g=admin&m=news&a=classify_del&cid=<?php echo ($lii["cid"]); ?>">删除</a>
                                </td>
                            </tr>
                            <?php if(!empty($lii["_child"])): if(is_array($lii["_child"])): $i = 0; $__LIST__ = $lii["_child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$liii): $mod = ($i % 2 );++$i;?><tr class="no <?php echo ($lii["cid"]); ?> p<?php echo ($li["cid"]); ?> p<?php echo ($lii["cid"]); ?>">
                                        <td><input class="check_box" type="checkbox" name="items" value="<?php echo ($liii["cid"]); ?>" /></td>
                                        <td class="tl ti40">
                                            <i class="flex"></i>
                                            <span><?php echo ($liii["cid"]); ?></span>
                                        </td>
                                        <td class="tl"><?php echo ($liii["cname"]); ?></td>
                                        <td><?php echo (date("Y-m-d H:i:s",$liii["addtime"])); ?></td>
                                        <td class="data_opt">
                                            <a href="?g=admin&m=news&a=classify_add&cid=<?php echo ($liii["cid"]); ?>">修改</a>
                                            <a class="confirm ajax-get" href="index.php?g=admin&m=news&a=classify_del&cid=<?php echo ($liii["cid"]); ?>">删除</a>
                                        </td>
                                    </tr><?php endforeach; endif; else: echo "" ;endif; endif; endforeach; endif; else: echo "" ;endif; endif; endforeach; endif; else: echo "" ;endif; ?>
            </table>
            <!-- 操作 -->
            <div class="opt">
                <a class="opt_btn" href="?g=admin&m=news&a=classify_add">添加</a>
