...
<body>
	...
	<!-- 主体 开始 -->
	<div class="main">
		<input id="url" type="hidden" value="?g=admin&m=news&a=announce_delall">
		<div class="wrapper">
			<!-- 主体头部 -->
			<div class="header">
				<h2>公告管理中心</h2>
				<a class="hover" href="?g=admin&m=news&a=notice">公告列表</a>
				<a href="?g=admin&m=news&a=notice_add">添加公告</a>
			</div>
			<!-- 主体搜索 -->
			<div class="search">
				<form class="form-horizontal" name="" action="?g=admin&m=news&a=notice" method="post">
					<p>筛选：</p>
					<select name="flags">
			...
			<table class="data">
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><?php echo ($li['is_display']?"显示":"不显示"); ?></td>
					<td><?php echo (date("Y-m-d h:i",$li["uptime"])); ?></td>
					<td class="data_opt">
						<a href="?g=admin&m=news&a=notice_add&id=<?php echo ($li["id"]); ?>">修改</a>
						<a class="confirm ajax-get" href="index.php?g=admin&m=news&a=announce_del&id=<?php echo ($li["id"]); ?>">删除</a>
					</td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			</table>
			<!-- 操作 -->
			<div class="opt">
				<a class="opt_btn" href="?g=admin&m=news&a=notice_add">添加</a>
