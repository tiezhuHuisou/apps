...
<!-- 主体 开始 -->
<div class="main">
    <input id="url" type="hidden" value="?g=admin&m=product&a=truckDelAll">
    <div class="wrapper">
        <!-- 主体头部 -->
        <div class="header">
            <h2>货车管理中心</h2>
            <a class="hover" href="?g=admin&m=product&a=truck">货车列表</a>
            <a href="?g=admin&m=product&a=truckAdd">添加货车</a>
        </div>
        <!-- 主体数据 -->
        <table class="data">
            <!-- 数据头部 -->
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><?php echo ($li['truck_type']); ?></td>
                    <td><?php echo ($li['truck_length']); ?></td>
                    <td><?php echo (date("Y-m-d H:i",$li['departure_time'])); ?></td>
                    <td><?php echo (date("Y-m-d H:i",$li['create_time'])); ?></td>
                    <td class="data_opt">
                        <a href="?g=admin&m=product&a=truckAdd&id=<?php echo ($li["id"]); ?>">修改</a>
                        <a class="confirm ajax-get" href="index.php?g=admin&m=product&a=truckDel&id=<?php echo ($li["id"]); ?>">删除</a>
                    </td>
                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
        </table>
        <!-- 操作 -->
        <div class="opt">
            <a class="opt_btn" href="?g=admin&m=product&a=truckAdd">添加</a>
