...
<body>
	...
	<!-- 主体 开始 -->
	<div class="main">
	<input id="url" type="hidden" value="?g=admin&m=banner&a=delall">
		<div class="wrapper">
			<!-- 主体头部 -->
			<div class="header">
				<h2>焦点图管理中心</h2>
				<a class="hover" href="?g=admin&m=banner">焦点图列表</a>
				<a href="?g=admin&m=banner&a=add">添加焦点图</a>
			</div>
			<!-- 主体数据 -->
			<table class="data">
				<!-- 数据头部 -->
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><?php echo ($li["classify"]); ?></td>
					<td><?php echo ($li["name"]); ?></td>
					<td><img src="<?php echo ($li["thumbnail"]); ?>" width="50" height="25" /></td>
					<td><?php echo (date("Y-m-d H:i",$li["addtime"])); ?></td>
					<td class="data_opt">
						<a href="?g=admin&m=banner&a=add&id=<?php echo ($li["id"]); ?>">修改</a>
						<a class="confirm ajax-get" href="index.php?g=admin&m=banner&a=del&id=<?php echo ($li["id"]); ?>">删除</a>
					</td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			</table>
			<!-- 操作 -->
			<div class="opt">
				<a class="opt_btn" href="?g=admin&m=banner&a=add">添加</a>
