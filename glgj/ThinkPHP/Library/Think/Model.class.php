...
class Model {
    ...
    public function regex($value,$rule) {
        $validate = array(
            ...
            'domain'    =>  '/^(?:[A-za-z0-9-]+\.)+[A-za-z]{2,4}(:\d+)?(?:[\/\?#][\/=\?%\-&~`@[\]\':+!\.#\w]*)?$/',
            'currency'  =>  '/^\d+(\.\d+)?$/',
            'number'    =>  '/^\d+$/',
            'zip'       =>  '/^\d{6}$/',
            'integer'   =>  '/^[-\+]?\d+$/',
            ...
            'mobile'    =>  '/^1\d{10}$/',
            'wechat'    =>  '/^[a-zA-Z\d_]{5,}$/',
            'telephone' =>  '/^(0\d{2,3}-\d{7,8}(-\d{1,6})?)|([4,8]00)-?(\d{3})-?(\d{4})$/',
            'phone'     =>  '/^(0\d{2,3}-\d{7,8}(-\d{1,6})?)|([4,8]00)-?(\d{3})-?(\d{4})|(1\d{10})$/',
            'password'  =>  '/^\w{6,16}|\w{32}$/',
            'truename'  =>  '/^([\x{4e00}-\x{9fa5}]{1,6}|[a-zA-Z]{1,6})$/u',
            'money'     =>  '/^\d+(\.\d{1,2})?$/',
        );
        // 检查是否有内置的正则表达式
    ...
    public function check($value,$rule,$type='regex'){
        ...
        switch($type) {
            ...
            case 'egt':
            case 'gt':
                return $type == 'gt' ? $value > $rule : $value >= $rule;
            case 'elt':
            case 'lt':
